    ...
    <section className="py-20 px-4 bg-gray-50">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-sm">
          {/* Table header */}
          <div className="grid grid-cols-3 bg-[#022b3d] text-white text-sm font-semibold px-6 py-3">
            <span>Equipment</span>
          ...
          {rentals.map(item => (
            <div
              ...
              className={[
                ...
                item.isHighlighted
                  ? 'bg-[#1a9fd4]/10 border-l-4 border-l-[#1a9fd4]'
                  : 'bg-white hover:bg-gray-50',
              ].join(' ')}
            >
              <div>
                <p className={`font-semibold text-sm ${item.isHighlighted ? 'text-[#1a9fd4]' : 'text-[#022b3d]'}`}>
                  {item.name}
                  {item.isHighlighted && (
                    <span className="ml-2 text-xs bg-[#1a9fd4] text-white px-2 py-0.5 rounded-full">Best Value</span>
                  )}
                </p>
                {item.description && (
              ...
              <div className="text-center">
                <p className="font-outfit font-bold text-[#1a9fd4] text-lg">{fmt(item.priceEGP)}</p>
                {item.priceUSD && (
