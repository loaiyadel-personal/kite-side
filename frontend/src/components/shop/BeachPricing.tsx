    ...
    <section className="py-20 px-4 bg-white">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-10">
          {beach.map(item => (
            <div key={item.id} className="text-center p-5 rounded-2xl border border-gray-100 shadow-sm hover:border-brand-primary/30 hover:shadow-md transition-all">
              <div className="text-3xl mb-3">{BEACH_ICONS[item.name] ?? '🏝️'}</div>
              <h3 className="font-semibold text-brand-dark text-sm mb-2 leading-tight">{item.name}</h3>
              <p className="font-outfit font-bold text-xl text-brand-primary">
                {fmt(item.priceEGP)}
        ...
        <div className="text-center bg-brand-dark/5 rounded-2xl p-6">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 px-8 py-3.5 rounded-full bg-brand-dark text-white font-semibold text-sm hover:bg-[#034a6a] transition-colors"
          >
            Book Your Day
          </a>
        </div>
